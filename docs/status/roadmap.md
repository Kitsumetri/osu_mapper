...
|---|-----------|----------------------|------|
| 1 | **Brute-force reward over ALL maps** | ✅ **DONE** — `measure_reward.py` (`--all`/`--workers`/`--bottom-n`/`--gold`/per-defect). Audit-1 (`n=87176`, all-ranked) drove the 3a-fix; audit-2 (`n=38140`, `--gold`) **confirmed: reward 0.97, playability 1.0, defect rate 0.0**, and its tail drove the grid broadening. | **USER:** refresh `corpus_stats` (recalibrates the broadened grid metric + bands the 4 new metrics), then a final `--all --gold` to confirm the grid lift. |
