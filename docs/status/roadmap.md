...
filter + per-defect reporting. 241 tests green, ruff clean. See [task_reward_flow](../versions/v9/task_reward_flow.md).

| # | candidate | status + what landed | next |
|---|-----------|----------------------|------|
| 1 | **Brute-force reward over ALL maps** | ✅ **tooling done + AUDIT RUN** (`n=87176`): `measure_reward.py` has `--all`, `--workers`, `--bottom-n`, and now `--gold` + per-defect reporting. The audit drove the 3a-fix recalibration (playability over-fired on gold). | **USER re-runs `--all --gold`** (command below) to confirm the recalibration: expect `playability` ≈ 1.0, reward ≈ 0.9+, `unhittable_jump` rate ≈ 0. |
