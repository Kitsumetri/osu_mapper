...
Five user ideas. **Round 3a (#1 tooling, #2, #3, #5) landed via three sub-agents on 2026-06-23 — code in
the working tree, 239 tests green, ruff clean, not yet committed or trained.** Detailed task reports under
[versions/v9/](../versions/v9/): [task_holdout_val](../versions/v9/task_holdout_val.md),
[task_reward_flow](../versions/v9/task_reward_flow.md),
[task_audio_features](../versions/v9/task_audio_features.md). **Two real bugs surfaced + fixed:** the
val-split audio leakage (folded into #2) and the inference mel pad/precision skew (folded into #5 rung 0).

| # | candidate | status + what landed | next |
|---|-----------|----------------------|------|
| 1 | **Brute-force reward over ALL maps** | ✅ **tooling done** (agent C): `measure_reward.py` gains `--all`/`--limit 0`, `--workers` (ProcessPoolExecutor), `--bottom-n` (low-reward-tail dump: path + reward + worst family). | **USER runs** the heavy scan to validate the new reward (command below). Read the low tail for blind spots — esp. single-BPM `on_quarter_grid_ratio` vs variable-BPM maps. |
...
| 3 | **Reward: rhythm ≫ flow + flow done right** | ✅ **implemented** (agent C). Family weights rhythm **2.0** / spacing_aim 1.0 / slider_shape 1.0 / flow **0.6** / accents 0.4 (rhythm 40%, flow 12% of quality); `on_quarter_grid_ratio` within-weight 2.0→3.0. New distributional metrics `stream_spacing_cv`, `slider_anchor_spread_px` (band-less until a gold-stats refresh; reward ignores band-less metrics). New bounded **multiplicative playability penalty** (no band needed): unhittable-jump velocity, unintended-stack, slider-overlap, degenerate-anchor — clean map → penalty 0 → reward unchanged. Flat-top anti-hacking + family balance preserved. **Caveat:** `on_quarter_grid_ratio` single-BPM (safe for generated maps). | USER: run #1 to confirm gold maps still score high; then rerun `corpus_stats` (heavy) to give the 2 new metrics bands. |
| 4 | **Early-abort sampling on reward trajectory** | ⏳ **not started.** `ddim_sample` exposes x0_hat every step (`diffusion.py:170,178`) → decode→reward at late steps, abort doomed best-of-N candidates. **Not** steering (eta=0 abort-only; reward non-diff). Monitor last ~25-40% (early x0_hat blurry); cheap proxy = `quality` only; decode in-memory; step-relative threshold. Modest payoff. | after #1 validates the reward. |
